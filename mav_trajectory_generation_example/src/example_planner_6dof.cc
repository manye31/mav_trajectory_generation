...
bool ExamplePlanner::planTrajectory(
  ...
  ROS_INFO("dimension is %d", dimension);
  bool success = false;
...
// Plans a trajectory from the current position to a middle point and goal position and velocity
bool ExamplePlanner::planTrajectory(
    const Eigen::VectorXd& goal_pos, const Eigen::VectorXd& goal_vel, const Eigen::VectorXd& mid_pos,
    mav_trajectory_generation::Trajectory* trajectory) {
  assert(trajectory);
  trajectory->clear();
  ...
  if (dimension == 6) 
  {
    ...
    Eigen::Vector3d goal_position = goal_pos.head(3);
    Eigen::Vector3d goal_lin_vel = goal_vel.head(3);
    Eigen::Vector3d mid_position = mid_pos.head(3);
    success = planTrajectory(
        goal_position, goal_lin_vel, mid_position, current_pose_.translation(),
        current_velocity_, max_v_, max_a_, &trajectory_trans);
    ...
    mav_msgs::vectorFromRotationMatrix(
        current_pose_.rotation(), &current_rot_vec);
    std::cout << "goal_position" << goal_pos << std::endl;
    std::cout << "goal_rotation" << goal_rotation << std::endl;
